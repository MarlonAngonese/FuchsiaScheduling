...
Thread* Scheduler::EvaluateNextThread(SchedTime now, Thread* current_thread, bool timeslice_expired,
  ...
  // Retain original checks for idle, active, and deadline states
  ...
  if (!fifo_run_queue_.empty()) {
      // If FIFO queue is not empty, prioritize it
      Thread* next_thread = fifo_run_queue_.front();
      fifo_run_queue_.pop_front();
      return next_thread;
  }

  if (is_deadline && is_new_deadline_eligible) {
      Thread* deadline_thread = FindEarlierDeadlineThread(now);
      if (deadline_thread) {
          deadline_run_queue_.erase(*deadline_thread);
